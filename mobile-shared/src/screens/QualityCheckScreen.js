...
export default function QualityCheckScreen() {
  ...
  const handleScanQR = async () => {
    // Triggers standard camera capture in search of a barcode/label
    try {
      if (NativeModules.CameraModule) {
        const path = await NativeModules.CameraModule.capturePhoto();
        // Since we are mock-resolving for barcode demo, extract any numeric value or default to a jar sequence
        Alert.alert('Scan Result', 'Label scanned successfully. Resolving jar data...');
        // Let's set a default sequential jar for testing or parse from OCR mock
        setJarNumber('1');
        fetchJarDetails(1);
      } else {
        Alert.alert('Unavailable', 'Native CameraModule not registered');
      }
    } catch (e) {
        ...
        <View>
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    ...
                    {log.details && (
                      <View style={styles.detailsBox}>
                        ...
                        {log.details.notes && (
                          <Text style={[styles.detailsText, { fontStyle: 'italic', marginTop: 4, color: '#38BDF8' }]}>
                            "{log.details.notes}"
                          </Text>
                        )}
          ...
          <View style={styles.card}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
              ...
              >
                <Text style={{ fontSize: 13, fontWeight: '700' }}>📸 Camera</Text>
              </TouchableOpacity>
              <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 22, fontWeight: '800', color: '#FFFFFF', marginBottom: 16 },
  warningBanner: {
  ...
  sectionTitle: { color: '#FFFFFF', fontSize: 14, fontWeight: '800', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
  label: { color: '#94A3B8', fontSize: 11, fontWeight: '800', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 },
  textInput: {
    flex: 1,
  ...
  aiTitle: { color: '#38BDF8', fontSize: 13, fontWeight: '800', marginBottom: 8 },
  aiText: { color: '#F1F5F9', fontSize: 13, lineHeight: 18 },
  timelineRow: {
    flexDirection: 'row',
  ...
  detailsText: { color: '#94A3B8', fontSize: 11 },
  grid: {
    flexDirection: 'row',
    gap: 12,
    marginBottom: 10
