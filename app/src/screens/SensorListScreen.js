...
export default function SensorListScreen({ navigation }) {
  ...
  // Alert notification tracking (1-hour cooldown per sensor)
  const notifiedAlertsRef = useRef({}); // { sensorId: lastNotifiedTimestamp }
  ...
  // Check for new alerts and fire push notification with 1-hour cooldown
  const checkAndNotifyAlerts = (roomsList, telemetryData) => {
    const now = Date.now();
    for (const room of roomsList) {
      ...
      const hasTemp = tempSensor && telemetryData[tempSensor.id];
      const temp = hasTemp ? parseFloat(telemetryData[tempSensor.id].temperature) : null;
      const hum = humSensor && telemetryData[humSensor.id] ? parseFloat(telemetryData[humSensor.id].humidity) : null;
      const isOnline = hasTemp ? telemetryData[tempSensor.id].is_online !== false : false;

      // Check offline
      if (hasTemp && !isOnline) {
        const sensorKey = `offline_${tempSensor.id}`;
        const lastNotified = notifiedAlertsRef.current[sensorKey] || 0;
        if (now - lastNotified > ALERT_COOLDOWN_MS) {
          triggerLocalNotification(
            `⚠️ ${room.name} Offline`,
            `Sensor ${room.name} has gone offline and stopped reporting data.`
          );
          notifiedAlertsRef.current[sensorKey] = now;
        }
        continue;
      }
      ...
      let alertMessages = [];
      if (tempSensor && temp !== null) {
        if (tempSensor.min_threshold !== null && temp < tempSensor.min_threshold) {
          alertMessages.push(`Temp too low: ${temp.toFixed(1)}°C (Min: ${tempSensor.min_threshold}°C)`);
        }
        if (tempSensor.max_threshold !== null && temp > tempSensor.max_threshold) {
          alertMessages.push(`Temp too high: ${temp.toFixed(1)}°C (Max: ${tempSensor.max_threshold}°C)`);
        }
      }
      if (humSensor && hum !== null) {
        if (humSensor.min_threshold !== null && hum < humSensor.min_threshold) {
          alertMessages.push(`Humidity too low: ${hum.toFixed(1)}%`);
        }
        if (humSensor.max_threshold !== null && hum > humSensor.max_threshold) {
          alertMessages.push(`Humidity too high: ${hum.toFixed(1)}%`);
        }
      }

      if (alertMessages.length > 0) {
        const sensorKey = `alert_${tempSensor.id}`;
        const lastNotified = notifiedAlertsRef.current[sensorKey] || 0;
        if (now - lastNotified > ALERT_COOLDOWN_MS) {
          triggerLocalNotification(
            `🚨 Alert: ${room.name}`,
        ...
        const sensorKey = `alert_${tempSensor.id}`;
        delete notifiedAlertsRef.current[sensorKey];
      }
    }
  };
  ...
  const fetchData = async () => {
    if (isOffline) return; // Don't poll when offline
    try {
      ...
      // Check for new alerts and send push notifications
      if (fetchedRooms.length > 0) {
        checkAndNotifyAlerts(fetchedRooms, telemetryMap);
      }
    } catch (e) {
      ...
      >
        ...
        {isOffline && (
          <View style={styles.offlineBanner}>
            <Text style={styles.offlineBannerText}>📡 You are offline — data may not be current</Text>
          </View>
        )}
        {showOnlineBanner && !isOffline && (
          <View style={styles.onlineBanner}>
            <Text style={styles.onlineBannerText}>✅ Back online</Text>
          </View>
        )}
